...
occs_path <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/quarter_3"
occs_path <- file.path(occs_path, "kelp_presence_3.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp_presence_3.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/quarter_3"
envs_path <- file.path(dir_envs_Mp, c('depth_3.tif', 'nitrate_nitrite_3.tif', 'phosphate_3.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth_3.tif', 'nitrate_nitrite_3.tif', 'phosphate_3.tif'),
  doBrick = FALSE)
...
ggplot(depth_dat, aes(depth_3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Depth",
...
ggplot(nitr_dat, aes(nitrate_nitrite_3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Nitrogen",
       y = "Predicted Habitat Suitability")
...
ggplot(phos_dat, aes(phosphate_3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Phosphate",
       y = "Predicted Habitat Suitability")
...
```{r}
terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/results/maxent-quarter-3-output.tif", overwrite = TRUE)
```