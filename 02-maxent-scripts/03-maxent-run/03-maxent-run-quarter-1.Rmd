...
occs_path <- "03-analysis-data/04-maxent-analysis/quarter-1"
occs_path <- file.path(data_dir, occs_path, "kelp-presence-1.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp-presence-1.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/03-analysis-data/04-maxent-analysis/quarter-1"
envs_path <- file.path(data_dir, dir_envs_Mp, c('depth-1.tif', 'nitrate-nitrite-1.tif', 'phosphate-1.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth-1.tif', 'nitrate-nitrite-1.tif', 'phosphate-1.tif'),
  doBrick = FALSE)
...
kable(vars)
tab_df(vars,
       title = "Model Variable Importance Quarter 1")
```
...
ggplot(depth_dat, aes(depth.1, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Depth",
...
ggplot(nitr_dat, aes(nitrate.nitrite.1, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Nitrogen",
       y = "Predicted Habitat Suitability")
...
ggplot(phos_dat, aes(phosphate.1, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Phosphate",
       y = "Predicted Habitat Suitability")
...
terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/03-analysis-data/04-maxent-analysis/results/maxent-quarter-1-output.tif", overwrite = TRUE)

terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/04-deliverables/05-maxent-outputs/maxent-quarter-1-output.tif", overwrite = TRUE)
```
