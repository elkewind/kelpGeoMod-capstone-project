...
occs_path <- "03-analysis-data/04-maxent-analysis/quarter-4"
occs_path <- file.path(data_dir, occs_path, "kelp-presence-4.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp-presence-4.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/03-analysis-data/04-maxent-analysis/quarter-4"
envs_path <- file.path(data_dir, dir_envs_Mp, c('depth-4.tif', 'nitrate-nitrite-4.tif', 'phosphate-4.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth-4.tif', 'nitrate-nitrite-4.tif', 'phosphate-4.tif'),
  doBrick = FALSE)
...
kable(vars)
tab_df(vars,
       title = "Model Variable Importance Quarter 4")
```
...
ggplot(depth_dat, aes(depth.4, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Depth",
...
ggplot(nitr_dat, aes(nitrate.nitrite.4, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Nitrogen",
       y = "Predicted Habitat Suitability")
...
ggplot(phos_dat, aes(phosphate.4, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Phosphate",
       y = "Predicted Habitat Suitability")
...
terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/03-analysis-data/04-maxent-analysis/results/maxent-quarter-4-output.tif", overwrite = TRUE)

terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/04-deliverables/05-maxent-outputs/maxent-quarter-4-output.tif", overwrite = TRUE)
```