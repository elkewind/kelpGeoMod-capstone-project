...
occs_path <- "03-analysis-data/04-maxent-analysis/quarter-3"
occs_path <- file.path(data_dir, occs_path, "kelp-presence-3.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp-presence-3.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/03-analysis-data/04-maxent-analysis/quarter-3"
envs_path <- file.path(data_dir, dir_envs_Mp, c('depth-3.tif', 'nitrate-nitrite-3.tif', 'phosphate-3.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth-3.tif', 'nitrate-nitrite-3.tif', 'phosphate-3.tif'),
  doBrick = FALSE)
...
kable(vars)
tab_df(vars,
       title = "Model Variable Importance Quarter 3")
```
...
ggplot(depth_dat, aes(depth.3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Depth",
...
ggplot(nitr_dat, aes(nitrate.nitrite.3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Nitrogen",
       y = "Predicted Habitat Suitability")
...
ggplot(phos_dat, aes(phosphate.3, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Phosphate",
       y = "Predicted Habitat Suitability")
...
terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/03-analysis-data/04-maxent-analysis/results/maxent-quarter-3-output.tif", overwrite = TRUE)

terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/google_drive_download/04-deliverables/05-maxent-outputs/maxent-quarter-3-output.tif", overwrite = TRUE)
```