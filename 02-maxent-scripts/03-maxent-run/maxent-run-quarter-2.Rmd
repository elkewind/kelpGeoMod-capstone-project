...
occs_path <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/quarter_2"
occs_path <- file.path(occs_path, "kelp_presence_2.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp_presence_2.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/quarter_2"
envs_path <- file.path(dir_envs_Mp, c('depth_2.tif', 'nitrate_nitrite_2.tif', 'phosphate_2.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth_2.tif', 'nitrate_nitrite_2.tif', 'phosphate_2.tif'),
  doBrick = FALSE)
...
ggplot(depth_dat, aes(depth_2, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Depth",
...
ggplot(nitr_dat, aes(nitrate_nitrite_2, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Nitrogen",
       y = "Predicted Habitat Suitability")
...
ggplot(phos_dat, aes(phosphate_2, layer)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Phosphate",
       y = "Predicted Habitat Suitability")
...
```{r}
terra::writeRaster(predSel_Mp, "/capstone/kelpgeomod/new_file_structure/03-analysis-data/04-maxent-analysis/results/maxent-quarter-2-output.tif", overwrite = TRUE)
```