...
occs_path <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/maxent/quarter_1"
occs_path <- file.path(occs_path, "kelp_presence_1.csv")
# get a list of species occurrence data
userOccs_Mp <- occs_userOccs(
  txtPath = occs_path, 
  txtName = "kelp_presence_1.csv", 
  txtSep = ",", 
  txtDec = ".")
...
dir_envs_Mp <- "/capstone/kelpgeomod/new_file_structure/03-analysis-data/maxent/quarter_1"
envs_path <- file.path(dir_envs_Mp, c('depth_1.tif', 'nitrate_nitrite_1.tif', 'phosphate_1.tif'))
# Create environmental object 
envs_Mp <- envs_userEnvs(
  rasPath = envs_path,
  rasName = c('depth_1.tif', 'nitrate_nitrite_1.tif', 'phosphate_1.tif'),
  doBrick = FALSE)
...
# Define path
# bgPath_Mp <- ""
# bgExt_Mp <- penvs_userBgExtent(
#   bgShp_path = paste0(bgPath_Mp, "aoi", ".shp"),
#   bgShp_name = paste0("aoi", c(".shp", ".shx", ".dbf")),
#   userBgBuf = 0,
#   occs = occs_Mp)
...
0.5, 4.5 range increasing by 0.2. Not using any categorical predictor
variables.
...
m_Mp <- model_Mp@models[["fc.LQHP_rm.0.5"]]
predSel_Mp <- predictMaxnet(m_Mp, bgMask_Mp,
                                          type = "cloglog", 
                                          clamp = FALSE)
...
#--------------------------------ENMEvaluate strategy------------------------------


model_enm <- ENMevaluate(
  occs = occs_Mp[, 3:4],
  bg = bgEnvsVals_Mp[, 2:3],
  user.grp = groups_Mp,
  envs = bgMask_Mp,
  ...
  parallel = TRUE,
  numCores = 6,
  taxon.name = "Macrocystis pyrifera",
...
aic.mod <- model_Mp@models[[which(model_MP@results$delta.AICc==0)]]
eval.variable.importance(model_Mp)
str(model_Mp, max.level=2)
